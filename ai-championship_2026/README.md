...
- 단계: `IDEA-051 사진이 묻는 하루` 공개 데모 배포 완료, 서버 키·실제 사진 검증 대기
- 기준일: 2026-09-14 (Asia/Seoul)
- 참가 접수 마감: 2026-09-18
- 과제 제출 마감: 2026-09-20
- 현재 출품 개발 1순위는 사용자 제안 `IDEA-051 사진이 묻는 하루`다. 사진 선택, 기억 질문 3개, 답변, 문장별 사진·사용자 근거가 있는 기록까지 구현했고 [공개 데모](https://my-project-xi-nine-28.vercel.app/)에서 첫 화면과 예시 앨범을 확인했다. Vercel 서버 키가 없어 실제 모델 호출은 아직 작동하지 않으므로 `공개 데모 서비스·실제 AI 활성화 대기`로 기록한다.
...
- 작동하는 MVP: 1개 (`RightsLedger 0.1`, 로컬·무저장) + 공개 데모 1개 (`사진이 묻는 하루`, 실제 AI 키 대기)
- 조건부 예비 1순위: `IDEA-004`
